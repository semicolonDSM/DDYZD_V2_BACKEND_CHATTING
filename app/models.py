...
class Room(db.Model):
    ...
    def writed(self, user_type):
        if user_type == 'C':
            if not self.user.is_in_room(self):
                self.user_looked = False
        else:
            if not self.club.club_head[0].user.is_in_room(self):
                self.club_looked = False
        db.session.commit()

    def update_room_message(self, msg, date):
    ...
    def json(self, is_user, index=0):
        ...
        return {
		    ...
		    "name" : name,
		    "image" : image,
		    "lastdate" :  isoformat(self.last_date if self.last_date is None else datetime(1,1,1,1,1,1,1)),
